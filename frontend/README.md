...
大文件传输默认走当前前端同源路径；如需绕过 Vite 开发代理并直连后端，可设置 `FRONTEND_TRANSFER_ORIGIN` / `VITE_TRANSFER_ORIGIN`，例如 `http://192.168.124.9:17878`。设置后上传票据和下载票据会使用该后端地址，后端 CORS 需要允许当前前端来源。该地址写错或不可达时不会自动猜测其他地址，也不会静默回退，前端会直接显示连接失败。
