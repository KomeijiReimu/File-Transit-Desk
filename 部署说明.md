...
- 下载票据绑定目录 ID、相对路径、文件大小、修改时间和可选 SHA-256 内容哈希，用于阻止同一路径文件被替换后继续复用旧授权。默认阈值内文件是内容级校验；超过阈值的大文件仍依赖大小和修改时间，除非把 `content_hash_max_mb` 设为 `0` 或调高阈值。
...
清理运行数据前请先停止后端服务，并确认已经备份需要保留的数据库或上传文件。默认情况下数据库位于 `backend/data/filetrans.db`；如果 `backend/config.yaml` 的 `database.path` 已修改，请以实际配置为准。

### 清空全部数据库记录

删除 SQLite 数据库文件后重启服务，后端会自动重新建表。这会清除会话、令牌、下载票据和访问记录，但不会删除上传目录中的文件。
...
也可以通过调小 `audit.retain` 后重启，让后端后续按保留条数裁剪访问记录；但立即清空仍建议使用上面的 SQL。

### 让所有登录和分享立即失效

如果需要应急止血，保留审计日志但清除所有登录会话、公开令牌和已兑换下载票据：
...
上传文件实际位置由 `storage.dirs[].path` 决定，不一定是默认的 `backend/uploads/`。确认路径后再删除，例如默认目录：

```bash
rm -rf backend/uploads/*
```

不要删除 `backend/config.yaml`，除非你希望重新生成并配置服务。
